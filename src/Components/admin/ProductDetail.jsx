                ...
                <table className="w-full text-left border border-collapse sm:border-separate border-pink-100 text-pink-400" >
                    ...
                    <tbody>
                        <tr>
                            ...
                            <th scope="col" className="h-12 px-6 text-md font-bold fontPara border-l first:border-l-0 border-pink-100 text-slate-700 bg-slate-100">Edit</th>
                        ...
                        {getAllProduct.map((item, index) => {
                            ...
                            return (
                                <tr key={id} className="text-pink-300">
                                    ...
                                    <td onClick={() => navigate(`/updateproduct/${id}`)} className="h-12 px-6 text-md transition duration-300 border-t border-l first:border-l-0 border-pink-100 stroke-slate-500 text-green-500 cursor-pointer " role="button" tabIndex={0} aria-label={`Edit ${title}`} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/updateproduct/${id}`); } }}>
                                        Edit
                                    </td>
                                    <td onClick={() => deleteProduct(id, title)} className="h-12 px-6 text-md transition duration-300 border-t border-l first:border-l-0 border-pink-100 stroke-slate-500 text-red-500 cursor-pointer " role="button" tabIndex={0} aria-label={`Delete ${title}`} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); deleteProduct(id, title); } }}>
                                        Delete
                                    </td>
                                </tr>
                            )
